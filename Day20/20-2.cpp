...
int main(){
    ...
    //Before anything we need to populate all the cheating directions
    //They are any sum of vectors in 20 possible combinations
    for(int i = 0; i <= 20; i++){
        for(int j = 0; j <= 20 - i; j++){
            if(i == 0 && j == 0) continue;
            cheatingDirections.push_back({i, j});
            if(i !=0){
                cheatingDirections.push_back({-i, j});
            }
            if(j != 0){
            ...
            for(int x = 0; x < distancesFromStartingPos[0].size(); x++){
                if(matrix[y][x]!= '#'){
                    for(ii direction : cheatingDirections){
                        ...
                        if(isEmpty(next)){
                            int newCheatingDistance = distancesFromStartingPos[y][x] + distances[next.second][next.first] + abs(direction.second) + abs(direction.first);
                            if(newCheatingDistance + 100 <= legalDistance){
                                cout << "Found better path  starting in ( " << x << "," << y << ") and ending in ( " << next.first<< "," << next.second<< ") " << endl;
                                total++;
                            }
                    }
                }
